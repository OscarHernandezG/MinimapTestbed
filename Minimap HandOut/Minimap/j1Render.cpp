...
// Blit to screen
bool j1Render::Blit(const SDL_Texture* texture, int x, int y, const SDL_Rect* section, float speed, double angle, int pivot_x, int pivot_y, float scale) const
{
	bool ret = true;
	float bScale = 0;
	if (scale == -1)
		bScale = App->win->GetScale();
	else
		bScale = scale;

	SDL_Rect rect;
	rect.x = (int)(camera.x * speed) + x * bScale;
	rect.y = (int)(camera.y * speed) + y * bScale;
